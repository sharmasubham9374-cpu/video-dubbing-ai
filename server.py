...
with open(WEB_DIR / "sitemap.xml", "w", encoding="utf-8") as f:
    f.write(EMBEDDED_SITEMAP)

with open(WEB_DIR / "robots.txt", "w", encoding="utf-8") as f:
    f.write(EMBEDDED_ROBOTS)

with open(WEB_DIR / "ads.txt", "w", encoding="utf-8") as f:
    f.write(EMBEDDED_ADS)
...
async def api_upload_video(file: UploadFile = File(...)):
    ...
    if not ext or ext not in [".mp4", ".mov", ".mkv", ".webm", ".avi", ".3gp", ".flv", ".m4v", ".ts"]:
        ext = ".mp4"
...
@app.get("/sitemap.xml")
def api_sitemap():
    return Response(content=EMBEDDED_SITEMAP, media_type="application/xml")

@app.get("/robots.txt")
def api_robots():
    return Response(content=EMBEDDED_ROBOTS, media_type="text/plain")

@app.get("/ads.txt")
def api_ads_txt():
    return Response(content=EMBEDDED_ADS, media_type="text/plain")
...
@app.get("/youtube-video-dubbing", response_class=HTMLResponse)
def page_youtube_dubbing():
    return HTMLResponse(content=EMBEDDED_HTML.replace("<title>VideoDubber AI", "<title>YouTube Video Hindi Dubbing AI - 1-Click Dubbing Studio"))

@app.get("/reels-hindi-dubber", response_class=HTMLResponse)
def page_reels_dubbing():
    return HTMLResponse(content=EMBEDDED_HTML.replace("<title>VideoDubber AI", "<title>Instagram Reels Hindi Dubbing AI - Bunty Voice Studio"))

@app.get("/shorts-dubbing-ai", response_class=HTMLResponse)
def page_shorts_dubbing():
    return HTMLResponse(content=EMBEDDED_HTML.replace("<title>VideoDubber AI", "<title>YouTube Shorts English to Hindi AI Dubber"))

@app.get("/bunty-ai-voice", response_class=HTMLResponse)
def page_bunty_voice():
    return HTMLResponse(content=EMBEDDED_HTML.replace("<title>VideoDubber AI", "<title>Bunty AI Voice Dubbing Generator - ElevenLabs Hindi Studio"))

@app.get("/free-ai-video-dubbing", response_class=HTMLResponse)
def page_free_dubbing():
    return HTMLResponse(content=EMBEDDED_HTML.replace("<title>VideoDubber AI", "<title>Free AI Video Dubbing Online - English to Hindi Dubber"))
